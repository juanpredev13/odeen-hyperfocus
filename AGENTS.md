...
Requires Docker running. `supabase/config.toml` does not exist yet — run `init` once.

```bash
# One-time: scaffold local config (creates supabase/config.toml)
npx supabase init

# One-time: link this repo to the remote project
npx supabase link --project-ref jpryskxhhoxphxfsbbux

# Start local stack (Postgres, Auth, Studio, etc. via Docker)
npx supabase start

# Apply supabase/migrations/*.sql to the local DB
npx supabase db reset

# Check status + local URLs/keys (API URL, anon key, service_role key, Studio URL)
npx supabase status
...
- After `start`, point `.env` at the **local** values from `supabase status` (not the remote project URL/keys) while developing locally
- New schema changes: add a file to `supabase/migrations/`, then `npx supabase db reset` to apply locally; `npx supabase db push` to apply to remote
- `supabase/.temp/` is gitignored — CLI internal state, safe to ignore
